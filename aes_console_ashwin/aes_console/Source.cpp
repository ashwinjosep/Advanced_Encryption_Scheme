...
int main()
{
	...
	{
		system("CLS");
		cout << "Enter the message in 16 characters\t:\t";
		getline(cin, input);
	...
	{
		system("CLS");
		cout << "Enter the key in 16 characters\t:\t";
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			...
			key.stateByte[i][j] = (byte)keyInput.at(4 * j + i);
		}
	}
	PrintState(s, 'h');

	// Replace input by sample input
	/*byte message[16] = 
	{
	...
	for (int i = 0, k=0; i < 4; i++, k+= 3)
	{
		for (int j = 0; j < 4; j++)
		{
			s.stateByte[i][j] = (byte)message[i+j+k];
			key.stateByte[i][j] = (byte)keyValue[i + j + k];
		}
	}
*/
	cout << endl << "Sample Input" << endl;
	PrintState(s, 'h');
	cout << endl << "Sample Key" << endl;
	PrintState(key, 'h');
	...
	cout << "\nRound Key Value\n";
	PrintWord(expandedKey[0]);
	PrintWord(expandedKey[1]);
	PrintWord(expandedKey[2]);
	PrintWord(expandedKey[3]);

	// Add Round Key
	s = AddRoundKey(s, expandedKey, 0, Nb);
	cout << "\nState after Round Key Addition\n";
	PrintState(s, 'h');
	...
	for (int round = 0; round < Nr-1; round++)
	{
		...
		cout << "\nRound Key Value\n";
		PrintWord(expandedKey[((round+1)*Nb)]);
		PrintWord(expandedKey[((round+1)*Nb)+1]);
		PrintWord(expandedKey[((round+1)*Nb)+2]);
		PrintWord(expandedKey[((round+1)*Nb)+3]);
	...
	// Expanded key should be of length Nb(Nr+1). Nb = 4 and Nr = 14 in AES256 implementation
	word expandedKey2[60];

	//perform key expansion
	keyExpansion(key, expandedKey2, Nk, Nb, Nr);

	cout << "\nRound Key Value\n";
	PrintWord(expandedKey2[0]);
	PrintWord(expandedKey2[1]);
	PrintWord(expandedKey2[2]);
	PrintWord(expandedKey2[3]);

	// Add Round Key
	s = AddRoundKey(s, expandedKey2, Nr*Nb, (Nr + 1)*(Nb - 1));
	cout << "\nState after Round Key Addition\n";
	PrintState(s, 'h');
	...
	for (int round = Nr - 2; round >= 0 ; round--)
	{
		...
		//cout << "\nRound Key Value\n";
		//PrintWord(expandedKey2[((round + 1)*Nb)]);
		//PrintWord(expandedKey2[((round + 1)*Nb) + 1]);
		//PrintWord(expandedKey2[((round + 1)*Nb) + 2]);
		//PrintWord(expandedKey2[((round + 1)*Nb) + 3]);

		//ADD ROUND KEY
		s = AddRoundKey(s, expandedKey2, (round + 1)*Nb, (round + 2)*Nb);
	...
	//ADD ROUND KEY 0, Nb
	s = AddRoundKey(s, expandedKey2, 0, Nb);
