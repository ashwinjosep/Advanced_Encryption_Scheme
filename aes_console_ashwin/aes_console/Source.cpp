...
private:
	...
	std::byte SubBytes2(std::byte in)
	{
		//u0 is msb, u7 is lsb
	...
	std::byte InvSubBytes2(std::byte in)
	{
		//u0 is msb, u7 is lsb
		//get rid of the other bits
		std::byte u0 = in >> 7;
	...
	word SubWord(struct word input)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			result.byteWord[i] = SubBytes2(input.byteWord[i]);
		}
		return result;
	}
	...
	struct state blockEncrypt(struct state s)
	{
		...
		for (int round = 0; round < Nr - 1; round++)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					s.stateByte[i][j] = SubBytes2(s.stateByte[i][j]);
					//std::byte temp1 = SubBytes(s.stateByte[i][j]);
					//std::byte temp2 = SubBytes2(s.stateByte[i][j]);
					//int pleaseGiveLineOfCode = 0;
				}
			}
	...
	struct state blockDecrypt(struct state s)
	{
		...
		for (int round = Nr - 2; round >= 0; round--)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					s.stateByte[i][j] = InvSubBytes2(s.stateByte[i][j]);
				}
			}
